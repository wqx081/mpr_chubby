...
void TableBuilder::WriteRawBlock(const StringPiece& block_contents,
  ...
  if (r->status.ok()) {
    ...
    trailer[0] = type;
    uint32 crc = hash::Value(block_contents.data(), block_contents.size());
    crc = hash::Extend(crc, trailer, 1);  // Extend crc to cover block type
    EncodeFixed32(trailer + 1, hash::Mask(crc));
