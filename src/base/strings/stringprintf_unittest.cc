...
TEST(PrintfTest, Empty) {
  EXPECT_EQ("", SPrintf("%s", string().c_str()));
  EXPECT_EQ("", SPrintf("%s", ""));
}
...
// MSVC does not support $ format specifier.
  ...
  EXPECT_EQ("123hello w", SPrintf("%3$d%2$s %1$c", 'w', "hello", 123));
#endif  // !COMPILER_MSVC
}
...
TEST(PrintfTest, Multibyte) {
  ...
  string value = SPrintf("%.*s", 3, kInvalidCodePoint);
...
TEST(PrintfTest, NoMultibyte) {
  ...
  string value = SPrintf("%.*s", 3, "\375\067s");
  setlocale(LC_CTYPE, old_locale);
...
TEST(PrintfTest, DontOverwriteErrno) {
  ...
  errno = ECHILD;
  string value = SPrintf("Hello, %s!", "World");
