#include "base/random/weighted_picker.h"
...
TEST(WeightedPicker, Simple) {
  PhiloxRandom philox(RandomSeed(), 17);
...
TEST(WeightedPicker, BigWeights) {
  PhiloxRandom philox(RandomSeed() + 1, 17);
  SimplePhilox rnd(&philox);
...
TEST(WeightedPicker, Deterministic) {
  ...
  TestPickAt(ARRAYSIZE(weights), weights);
}

TEST(WeightedPicker, Randomized) {
  PhiloxRandom philox(RandomSeed() + 10, 17);
  SimplePhilox rnd(&philox);
  TestPicker(&rnd, 1);
  TestPicker(&rnd, 2);
  TestPicker(&rnd, 3);
