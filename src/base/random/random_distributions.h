#ifndef BASE_RANDOM_RANDOM_DISTRIBUTIONS_H_
#define BASE_RANDOM_RANDOM_DISTRIBUTIONS_H_
...
float Uint32ToFloat(uint32 x);
double Uint64ToDouble(uint32 x0, uint32 x1);
...
inline void BoxMullerFloat(uint32 x0, uint32 x1, float* f0, float* f1) {
...
// under the unit normal distribution.
inline void BoxMullerDouble(uint32 x0, uint32 x1, uint32 x2, uint32 x3, double* d0,
                     double* d1) {
  // This function implements the Box-Muller transform:
  // http://en.wikipedia.org/wiki/Box%E2%80%93Muller_transform#Basic_form
  // Do not send a really small number to log().
...
inline float Uint32ToFloat(uint32 x) {
...
inline double Uint64ToDouble(uint32 x0, uint32 x1) {
...
#endif  // BASE_RANDOM_RANDOM_DISTRIBUTIONS_H_
