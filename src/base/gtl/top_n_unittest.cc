...
using base::gtl::TopN;
using base::random::PhiloxRandom;
using base::random::SimplePhilox;
using base::string;
...
void TestIntTopNHelper(size_t limit, size_t n_elements, const Cmp &cmp,
  ...
  for (size_t i = 0; i != n_elements; ++i) shadow[i] = random->Uniform(limit);
...
TEST(TopNTest, Ptr) {
  ...
  base::gtl::STLDeleteElements(&extract);
}
